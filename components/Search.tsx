    ...
    <div className="xl:w-96">
      <div className="input-group relative flex flex-wrap items-stretch w-full">
        ...
        {availableProducts.length > 0 && (
          <ul
            className="absolute bg-white pr-8 pl-5 rounded z-0 focus:shadow focus:outline-none text-xl py-5 border border-stone-500 no-scrollbar overflow-scroll w-full"
            style={{ top: 37, maxHeight: 300, zIndex: 10000 }}
          >
            {availableProducts.map((item) => (
              <li
                className="px-1 py-2 border-b-2"
                onClick={() => redirectToProducts(item.title)}
              >
                {item.title}
              </li>
            ))}
